...
pub(crate) async fn run_install(
    ...
    let skill_spec = name.ok_or_else(|| {
        AgentError::Config("Missing skill name. Usage: hermes skills install <name>".into())
    })?;
    let (skill_name, _requested_version) = skills_infra::parse_skill_name_and_version(&skill_spec);
    ...
    let (files, install_seed, provenance) = if let Some((source, key)) = registry_prefixed {
        match source.as_str() {
            "official" => {
                ...
                let resolved = skills_infra::resolve_official_skill_source(&client, &key).await?;
                println!(
                ...
                (
                    ...
                    skills_infra::SkillInstallProvenance {
                        ...
                        trust_level: skills_infra::default_trust_level_for_source("official")
                            .to_string(),
            ...
            "skills.sh" => {
                let install_key = key.clone();
                let resolved = skills_infra::resolve_skills_sh_source(&client, &key).await?;
                println!(
                ...
                (
                    ...
                    skills_infra::SkillInstallProvenance {
                        ...
                        identifier: key.clone(),
                        trust_level: skills_infra::default_trust_level_for_source("skills.sh")
                            .to_string(),
                        metadata: serde_json::json!({
                            "repo": resolved.repo,
                            "branch": resolved.branch,
            ...
            "lobehub" => {
                ...
                (
                    ...
                    skills_infra::SkillInstallProvenance {
                        ...
                        identifier: key,
                        trust_level: skills_infra::default_trust_level_for_source("lobehub")
                            .to_string(),
                        metadata: serde_json::json!({}),
                    },
                )
            ...
            "clawhub" => {
                ...
                (
                    ...
                    skills_infra::SkillInstallProvenance {
                        ...
                        identifier: key,
                        trust_level: skills_infra::default_trust_level_for_source("clawhub")
                            .to_string(),
                        metadata: serde_json::json!({}),
                    },
                )
        ...
        (
            ...
            skills_infra::SkillInstallProvenance {
                source: "github".to_string(),
                identifier,
                trust_level: skills_infra::default_trust_level_for_source("github").to_string(),
                metadata: serde_json::json!({
                    "repo": resolved.repo,
                    "branch": resolved.branch,
                    "skill_dir": resolved.skill_dir,
    ...
    {
        let resolved =
            skills_infra::resolve_skill_in_repo(&client, &skill_name, skill_hint, Some("skills"))
                .await?;
        println!(
            "Resolved source: {}/{} @ {}",
            resolved.repo, resolved.skill_dir, resolved.branch
        ...
        let from_index =
            skills_infra::resolve_skill_via_registry_index(&client, &skill_name, None).await;
        if let Ok(hit) = from_index {
            if hit.source.eq_ignore_ascii_case("official") {
                let resolved =
                    skills_infra::resolve_official_skill_source(&client, &hit.identifier).await?;
                println!(
                ...
                match hit.install_source {
                    skills_infra::RegistryInstallSource::GitHub(resolved) => {
                        let branch =
                            skills_infra::github_default_branch(&client, &resolved.repo).await?;
                        let resolved = skills_infra::ResolvedSkillSource { branch, ..resolved };
                        println!(
                            "Resolved source [{}]: {}/{} @ {}",
            ...
            let (resolved, route) =
                skills_infra::resolve_install_via_fallback_router(&client, &skill_name, &taps)
                    .await?;
            match route {
            ...
            (
                ...
                skills_infra::SkillInstallProvenance {
                    source: match route {
                        skills_infra::InstallFallbackSource::SkillsSh => "skills.sh".to_string(),
                        skills_infra::InstallFallbackSource::Tap => "tap".to_string(),
                    },
                    identifier: format!("{}/{}", resolved.repo, resolved.skill_dir),
                    trust_level: skills_infra::default_trust_level_for_source(match route {
