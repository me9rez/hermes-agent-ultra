...
pub(super) fn plugin_git_host_allowed(url: &str, allow_untrusted: bool) -> bool {
    if allow_untrusted {
        return true;
    }
...
pub(super) fn scan_plugin_security(root: &std::path::Path) -> Vec<String> {
    let mut out = Vec::new();
...
pub(super) async fn git_checkout_ref(
